...
export function FieldEditorDrawer({
  ...
  const impactRules = useMemo(() => {
    ...
    const currentId = normalize(field.fieldId || field.ctgFieldName);
    const currentName = (field.fieldName || "").toLowerCase();
    const currentDisplay = (field.fieldDisplayName || field.displayName || "").toLowerCase();

    availableFields.forEach(af => {
      const raw = af.visibilityConditionObject || af.visibilityConditions || af.visibilityCondition;
      if (!raw) return;
      try {
        const parsed = typeof raw === 'string' ? JSON.parse(raw) : raw;
        const findInRules = (rules: any[]): boolean => {
          return rules.some(r => {
            if (Array.isArray(r.rules)) return findInRules(r.rules);
            const rId = normalize(r.conditionFieldId || r.fieldId || r.field?.id || "");
            const rLabel = (r.label || r.displayName || r.field?.label || r.field?.displayName || "").toLowerCase();
            return (rId && rId === currentId) || (rLabel && (rLabel === currentName || rLabel === currentDisplay));
          });
        };

        if (parsed.rules && findInRules(parsed.rules)) {
           results.push({
             ...af,
             logicDescription: `${parsed.condition || "AND"} logic triggered by this field`
           });
        }
      } catch (e) {}
    });
